...
var error = document.getElementsByClassName('errorAlert');
...
formValidate.addEventListener('submit', function (event){
    ...
    if(cardNum.value.length < 13 || cardNum.value.length > 19 || cardNum.value.length === ''){
        error.innerHTML = "Please enter a valid CC Number"

    if(cardCVC.value.length !== 3 || cardCVC.value.length > 3 || cardCVC.value.length === ''){
        error.innterHTML = "Please enter a valid CVC Number"
    }  
